...
INVENTORY = SCRIPT_DIR / "p101-test-inventory.json"
GRAPH = SCRIPT_DIR / "p101-check-graph.json"
...
def repository_rows(path: Path) -> list[tuple[str, Path, str]]:
    ...
    for number, raw in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        ...
        repository = (SCRIPT_DIR / relative).resolve()
...
def validate(document: dict[str, Any], graph: dict[str, Any]) -> dict[str, int]:
    ...
    for raw in exclusions_raw:
        ...
        if not (SCRIPT_DIR / path).is_file():
            raise InventoryError(f"stale verification exclusion: {path}")

    repository_count = 0
    repository_entry_count = 0
    for _url, repository, language in repository_rows(SCRIPT_DIR / manifest_name):
        repository_count += 1
    ...
    governed.add("p101-check-graph.py")
    for path in governed:
        entry = SCRIPT_DIR / path
