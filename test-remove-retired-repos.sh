...
mkdir -p "$sandbox/workspace/scripts" "$sandbox/workspace/libraries" "$sandbox/remotes"
cp ./remove-retired-repos.sh "$sandbox/workspace/scripts/"
chmod +x "$sandbox/workspace/scripts/remove-retired-repos.sh"
...
(
  ...
  ./remove-retired-repos.sh > "$sandbox/dry-run.out" 2> "$sandbox/dry-run.err" || status=$?
  [[ "${status:-0}" -eq 1 ]]
)
...
(
  cd "$sandbox/workspace/scripts"
  printf 'q\n' | ./remove-retired-repos.sh --apply --yes --interactive \
...
(
  cd "$sandbox/workspace/scripts"
  ./remove-retired-repos.sh --apply --yes > "$sandbox/final.out"
)
