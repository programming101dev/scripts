...
def active_repository_paths() -> list[Path]:
    active: list[Path] = []
    for raw_line in (SCRIPT_DIR / "repos.txt").read_text().splitlines():
        line = raw_line.strip()
        if not line or line.startswith("#"):
            continue
        fields = line.split("|")
        if len(fields) >= 2:
            active.append((SCRIPT_DIR / fields[1]).resolve())
    return active
...
def main() -> int:
    ...
    central_rows = table(SCRIPT_DIR / "wrapper-library-map.tsv")
