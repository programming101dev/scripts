...
This repository owns executable fault and mutation campaigns.
Its internal engines are `test-faults` and `test-mutation`.

- Inputs: a command under test, the wrapper fault contract, mutation
  candidates, and bounded campaign options.
- Outputs: per-case evidence, surviving/killed mutation results, receipts, and
  status.
