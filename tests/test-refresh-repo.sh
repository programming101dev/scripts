...
P101_GIT_RETRY_ATTEMPTS=1 ./distribution/refresh-repo.sh "$consumer" > "$sandbox/current.out"
...
status=0
P101_GIT_RETRY_ATTEMPTS=1 ./distribution/refresh-repo.sh "$consumer" > "$sandbox/updated.out" || status=$?
[[ "$status" -eq 1 ]]
...
git -C "$publisher" push --quiet
published_head="$(git -C "$publisher" rev-parse HEAD)"
mkdir -p "$sandbox/clone-driver/distribution"
...
status=0
P101_GIT_RETRY_ATTEMPTS=1 ./distribution/refresh-repo.sh "$consumer" \
...
mkdir -p "$sandbox/interactive-driver/distribution"
cp ./distribution/clone-repos.sh "$sandbox/interactive-driver/distribution/"
chmod +x "$sandbox/interactive-driver/distribution/clone-repos.sh"
printf '%s|%s|c\n' "$remote" "$consumer" \
    > "$sandbox/interactive-driver/repos.txt"
cat > "$sandbox/interactive-driver/distribution/refresh-repo.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
EOF
chmod +x "$sandbox/interactive-driver/distribution/refresh-repo.sh"
