...
check_case() {
  ...
  set +e
  ./p101-analyze.py -o "$analysis" --model-tool "$model_tool" "$capture" \
    >> "$log" 2>&1
  analysis_status=$?
  set -e
  ...
  if ! ./p101-model.py verify -e "$expectation" "$analysis" >> "$log" 2>&1; then
...
if ./test-p101-runtime.py >> "$log_dir/policy-unit.log" 2>&1; then
  echo "    PASS"
