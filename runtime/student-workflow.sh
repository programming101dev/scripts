...
doctor_tool="$(find_tool P101_AUDIT_DOCTOR "$(last_build_tool "$workspace_dir/programs/p101-audit" audit-doctor)" audit-doctor)" || { echo "student workflow: audit-doctor not found" >&2; exit 2; }
doctor_dir="$(CDPATH='' cd -- "$(dirname -- "$doctor_tool")" && pwd -P)"
wrapper_tool="$(find_tool P101_AUDIT_WRAPPERS "$doctor_dir/audit-wrappers" "$workspace_dir/programs/p101-audit/audit-wrappers" audit-wrappers)" || { echo "student workflow: audit-wrappers not found" >&2; exit 2; }
error_contract_tool="$(find_tool P101_AUDIT_ERRORS "$doctor_dir/audit-errors" "$(last_build_tool "$workspace_dir/programs/p101-audit" audit-errors)" audit-errors)" || { echo "student workflow: audit-errors not found" >&2; exit 2; }
module_tool="$(find_tool P101_AUDIT_MODULES "$doctor_dir/audit-modules" "$(last_build_tool "$workspace_dir/programs/p101-audit" audit-modules)" audit-modules)" || { echo "student workflow: audit-modules not found" >&2; exit 2; }
