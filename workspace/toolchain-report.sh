...
case " $* " in
  *" --help "*|*" -h "*)
    cat <<'P101_USAGE'
toolchain-report.sh — collect a portable report of this machine's toolchain.
...
learns is written under toolchain-report/ in the scripts repository so it can
be inspected locally or attached to a diagnostic report.
...
# FreeBSD). It is READ-ONLY with respect to the build system: it never edits
# flags/*.txt, supported_*.txt, sanitizers.txt, or version.txt. Everything it
# learns is written under toolchain-report/ in the scripts repository so it
# can be inspected locally or attached to a diagnostic report.
#
# What it collects:
#   system.txt        OS / arch / core count
#   tools.txt         cmake, clang-format, clang-tidy, cppcheck, python3
...
# ---------- support tools ----------
{
  ...
  if [ -x ./workspace/check-env.sh ]; then
    ./workspace/check-env.sh 2>&1
    echo "exit: $?"
  else
...
if [ -x ./generators/harvest-flags.py ] && have python3 && [ -n "$working_compilers" ]; then
  note "harvesting flag universes (this queries the installed compilers)..."
  # shellcheck disable=SC2086
  python3 ./generators/harvest-flags.py $working_compilers > "$OUT/harvest-summary.txt" 2>&1 || true
...
echo "Attach this folder when sharing a reproducible toolchain diagnosis."
echo "=================================================================="
