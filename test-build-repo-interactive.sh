...
cat > "$sandbox/scripts/refresh-repo.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
exit 1
EOF
chmod +x "$sandbox/scripts/refresh-repo.sh"
...
cp ./refresh-repo.sh "$sandbox/matrix/refresh-repo.sh"
chmod +x "$sandbox/matrix/update-all.sh" "$sandbox/matrix/pull.sh" \
  "$sandbox/matrix/refresh-repo.sh"
